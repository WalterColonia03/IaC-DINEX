...
resource "aws_lambda_function" "notifications" {
 filename         = "${path.module}/../../application/lambda/notifications/deployment.zip"
  function_name    = "${var.project}-notifications-${var.environment}"
  role            = aws_iam_role.lambda_role.arn
  handler         = "index.handler"
  runtime         = "python3.11"
  source_code_hash = filebase64sha256("${path.module}/../../application/lambda/notifications/deployment.zip")
  ...
  dashboard_body = jsonencode({
    widgets = [
      ...
      # Widget 2: Métricas de DynamoDB
      {
        ...
        properties = {
          metrics = [
            ["AWS/DynamoDB", "ConsumedReadCapacityUnits", "TableName", aws_dynamodb_table.tracking.name, { stat = "Sum", label = "Read Capacity" }],
            [".", "ConsumedWriteCapacityUnits", "TableName", aws_dynamodb_table.tracking.name, { stat = "Sum", label = "Write Capacity" }]
