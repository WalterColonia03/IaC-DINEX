...
resource "aws_lambda_function" "tracking" {
  ...
  filename = "${path.module}/../../application/lambdas/tracking/deployment.zip"
  ...
  source_code_hash = filebase64sha256("${path.module}/../../application/lambdas/tracking/deployment.zip")
...
resource "aws_lambda_function" "notifications" {
 filename         = "${path.module}/../../application/lambdas/notifications/deployment.zip"
  function_name    = "${var.project}-notifications-${var.environment}"
  role            = aws_iam_role.lambda_role.arn
  handler         = "index.handler"
  runtime         = "python3.11"
  source_code_hash = filebase64sha256("${path.module}/../../application/lambdas/notifications/deployment.zip")
